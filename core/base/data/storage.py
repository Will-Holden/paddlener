...
def storage(storage_config, index, max_lenth=10000):
    """ Given the configuration for storage and the index, return the
    configured storage instance.
    """
    if 'dict' in storage_config:
        return InMemoryStorage(storage_config['dict'], max_lenth)
...
class InMemoryStorage(BaseStorage):
    def __init__(self, config, max_length=10000):
        self.name = 'dict'
        self.storage = dict()
    ...
    def append_val(self, key, val):
        ...
        if len(self.storage[key]) >= self.max_length:
            print("warnning, length exceed max lenth, will abuse one random data")
